...
for index in range(len(seq_path)):
    ...
    for i in range(len(img_path)):
        ...
        sess = rt.InferenceSession("best_0303_sim_qt_1.onnx", None)  #best_718_sim_qt_PPQ.onnx0.8425  best_718_sim.onnx 0.878
                                                         #best_824_sim_qt (3).onnx0.806   #best_sim_QT.onnx 0.8526
        #best_0106.onnx 0.9136 best_0106_f16.onnx 0.91435
        input_name0 = sess.get_inputs()[0].name
        input_name1 = sess.get_inputs()[1].name
        out_name = sess.get_outputs()[0].name
        pred_onx = sess.run([out_name], {input_name0: template, input_name1: search})  # 执行推断
        pred_onx =torch.squeeze(torch.as_tensor(np.array(pred_onx)),dim=1)

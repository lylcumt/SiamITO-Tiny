...
def val(val_data_path, val_model, save_video=True, vis=False):
    ...
    for index in range(len(seq_path)):
        ...
        for i in range(len(img_path)):
            ...
            # x = search[:, 0, :, :].view(search.shape[2], search.shape[3]).data.cpu().numpy()
            # x -= np.min(x)
            # x /= np.max(x)
            # cv2.imshow('feature', x)
            # cv2.imshow('search_region', search_region)

            # output = net.match_corr(template, search)
            ...
            # if i%100==0:
            #     print(str(index)+" "+str(x_t)+" "+str(y_t))
            ...
            if vis:
                ...
                cv2.putText(o_search, 'Frame:'+str(i), (0, 25),
                           cv2.FONT_HERSHEY_TRIPLEX, 1.0, (255, 255, 255), 1)
                cv2.imshow('test', o_search)
                filename = str(index)+"_"+str(i)
                cv2.imwrite('./output/'+filename+'.jpg',o_search)
...
if __name__ == '__main__':
    val_data = '../SiamITO1227/infrared_small_object_data/val'

    device = torch.device("cuda")
    val_net = torch.load('best0106.pkl')
    val_net.to(device)
    val_net.eval()

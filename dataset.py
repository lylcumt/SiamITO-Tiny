...
class Pair(Dataset):
    def __init__(self, data_path, transforms=None, rand_choice=True, gary=True,
                 img_size=256, search_size=55, template_size=19, r_pos=7):
...
if __name__ == '__main__':
    data_path = './infrared_small_object_data/train'

    transform = transforms.ToTensor()

    dataset = Pair(data_path, transforms=transform, rand_choice=True, gary=True,
                   search_size=65, template_size=25, r_pos=6)
    for i in range(1000):
        dataset.__getitem__(1)
    # dataloder = DataLoader(dataset, batch_size=1, shuffle=True)
    # print(len(dataloder))
    # for index, (i, labels) in enumerate(dataloder):
    #     print(i.shape, labels.shape)

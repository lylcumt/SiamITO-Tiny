...
class SiameseITO(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        match_map = self.match_corr(embedding_template, cross_attn_search)
    ...
    def match_corr(self, embed_tem, embed_srh):
        ...
        if self.upscale:
            match_map = fct.interpolate(match_map, self.upscale_size, mode='nearest')
...
class CrossAtt(nn.Module):
    ...
    def forward(self, main_feature, cross_feature):
        ...
        z = fct.interpolate(main_feature, cross_size, mode='nearest')
...
if __name__ == '__main__':
    testnet = Backbone()
    net = SiameseITO(testnet)
    cross = CrossAtt(128)
    non = SelfAtt(in_channels=128)

    template = randn(8, 1, 25, 25)
    search = randn(8, 1, 55, 55)
